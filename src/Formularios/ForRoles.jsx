...
// Construye un objeto permisos vacío en base a PANTALLAS/ACCIONES
...
// Normaliza un JSON existente a la forma esperada por el form
...
ACCIONES.forEach((a) => {
    base[p.key][a.key] = current[a.key] === true;
});
});
...
// Rehidratar cuando cambie el registro a editar
useEffect(() => {
setForm((prev) => ({
    ...prev,
    nombre: initialData?.nombre || "",
...
setForm((prev) => {
    ...
    if (checked) {
    // Si activas acceso total, enciende todos los flags
    PANTALLAS.forEach((p) =>
        ACCIONES.forEach((a) => (next.permisos[p.key][a.key] = true))
    );
    }
    return next;
});
...
ACCIONES.every((a) => form.permisos[moduleKey]?.[a.key] === true);
...
setForm((prev) => {
    ...
    ACCIONES.forEach((a) => (next.permisos[moduleKey][a.key] = checked));
    return next;
});
};
    ...
    <div className="fc-field col-span-full">
        ...
        {PANTALLAS.map((mod) => (
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                {ACCIONES.map((a) => (
                <label key={a.key} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                    <input
                    type="checkbox"
