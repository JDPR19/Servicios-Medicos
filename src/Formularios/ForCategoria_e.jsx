...
function ForCategoria({ initialData = {}, onSave, onClose }) {
    ...
    const handleSave = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await axios.post(`${BaseUrl}categoria_e/registrar`, form, { headers: getAuthorization() });
            ShowToast?.('Categoria registrada con exito', 'success');
            if (onSave) onSave(response.data);
            if (onClose) onClose();
        } catch (error) {
            const msg = error?.response?.data?.message || 'error registrando categoria';
            ShowToast?.(msg, 'error');
        } finally {
            setLoading(false);
        }
    ...
    const handleEdit = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await axios.put(`${BaseUrl}categoria_e/actualizar/${initialData.id}`, form, { headers: getAuthorization() });
